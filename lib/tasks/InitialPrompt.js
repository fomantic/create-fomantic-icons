...
function selectIconSetVersion(iconSet, accessToken) {
    ...
    return new Promise(function (resolve) {
        axios_1.default
            .get("https://api.github.com/repos/".concat(iconSet.repo, "/releases?page=1&per_page=50"), axiosConfig)
            .then(function (_a) {
            ...
            if (status === 200) {
                ...
                qoa_1.default
                    .interactive({
                    handle: 'setVersion',
                    query: "Which version of ".concat(iconSet.name, " should we download?"),
...
function askForDistPath() {
    return new Promise(function (resolve) {
        Logger_1.default.log();
        var cwd = process.cwd();
        var defaultPath = (0, path_1.resolve)(cwd, 'fui-icons');
        qoa_1.default
            .input({
            type: 'input',
            query: "Where would you like the dist saved? [".concat(defaultPath, "]"),
            handle: 'distPath',
        })
            .then(function (_a) {
            var distPath = _a.distPath;
            resolve(distPath === ''
                ? defaultPath : (0, path_1.resolve)(distPath));
        })
            .catch(function (err) {
            Logger_1.default.error(err);
            process.exit(1);
